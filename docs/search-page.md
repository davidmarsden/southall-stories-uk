# Search

Search the Southall Stories archive for people, places, organisations, developments, campaigns and issues covered over the years.

The search box above uses Micro.blog’s site index. It is useful for finding words and names that may appear deep inside older reporting, but it is not a substitute for the organised archive and category pages.

## Good things to search for

Try a specific name, place or issue rather than a broad term. For example:

- **Southall Gasworks** or **Berkeley Group**
- **Peter Mason** or another councillor or public figure
- **children’s centres**
- **air pollution** or **naphthalene**
- **fly-tipping** or **bin collections**
- **Section 106** or **planning**
- **Southall Town Hall**

## Other ways to browse

If you are looking for a body of work rather than one particular reference, these may be easier:

- [Investigations →](/categories/investigations/)
- [Local democracy →](/categories/local-democracy/)
- [Environment & health →](/categories/environment/)
- [Campaigns →](/campaigns/)
- [Full archive →](/archive/)

Search results reflect what Micro.blog has indexed from the site. If you know something exists but cannot find it here, the [full archive](/archive/) is the best fallback.
